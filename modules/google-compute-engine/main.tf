...
locals {
  ...
  cp_node_networking = [
    for n in range(var.worker_node_count) : templatefile("${path.module}/templates/node_networking.py", {
      local_ip : element(google_compute_instance.cp_node.*.network_interface.0.network_ip, n),
      gre_ip_cidr : format("%s/%s", cidrhost(var.private_subnet, n + 2), split("/", var.private_subnet).1),
  ...
  worker_node_networking = [
    for n in range(var.worker_node_count) : templatefile("${path.module}/templates/node_networking.py", {
      local_ip : element(google_compute_instance.worker_node.*.network_interface.0.network_ip, n),
      gre_ip_cidr : format("%s/%s", cidrhost(var.private_subnet, n + 5), split("/", var.private_subnet).1),
      cp_local_ip_list : jsonencode(google_compute_instance.cp_node.*.network_interface.0.network_ip),
      worker_local_ip_list : jsonencode(google_compute_instance.worker_node.*.network_interface.0.network_ip),
      gre_cidr : var.private_subnet
    })
...
resource "null_resource" "cp_node_networking" {
  ...
  provisioner "file" {
    content     = element(local.cp_node_networking.*, count.index)
...
resource "null_resource" "worker_node_networking" {
  ...
  provisioner "file" {
    content     = element(local.worker_node_networking.*, count.index)
    destination = "/home/${local.username}/bootstrap/node_networking.py"
  }
